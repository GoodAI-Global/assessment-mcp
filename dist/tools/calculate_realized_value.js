...
export const CALCULATE_REALIZED_VALUE_TOOL = {
    ...
    description: "Calculates and tracks the actual value realized from AI implementations compared to projected business case. Provides comprehensive ROI analysis, value breakdown, and executive-ready metrics.",
    inputSchema: {
        type: "object",
        properties: {
...
// ============================================
function calculateMonthsSinceGoLive(goLiveDate, measurementDate) {
...
function calculateValueBreakdown(actual, hourlyRate = 75) {
    // Cost savings
...
function calculateProjectionComparison(projected, breakdown, _monthsSinceGoLive) {
    ...
    const benefitsAchieved = projected.key_projected_benefits.map((benefit) => {
        ...
        return {
            benefit,
            status: achieved ? "achieved" : "partially_achieved",
            notes: achieved
...
function calculateInvestmentAnalysis(implementation, totalMonthlyValue, monthsSinceGoLive) {
...
function assessIntangibleValue(intangible, annualizedValue) {
...
function assessChallenges(challenges, monthlyValue, monthsSinceGoLive) {
    if (!challenges) {
        return {
...
function calculateValueTrajectory(monthlyValue, totalInvestment, ongoingMonthlyCost, monthsSinceGoLive, goLiveDate) {
...
function generateExecutiveMetrics(summary, breakdown, investment) {
...
function generateRecommendations(summary, comparison, challenges, futureContext) {
    const optimization = [];
    const expansion = [];
...
// ============================================
export function calculateRealizedValue(input) {
    const { project_name, client_name, implementation_info, projected_values, actual_values, intangible_benefits, challenges, future_context, } = input;
    const monthsSinceGoLive = calculateMonthsSinceGoLive(implementation_info.go_live_date, implementation_info.measurement_date);
    ...
    // Value trajectory
    const valueTrajectory = calculateValueTrajectory(totalMonthlyValue, implementation_info.implementation_cost_usd, implementation_info.ongoing_monthly_cost_usd, monthsSinceGoLive, implementation_info.go_live_date);
    // Build value summary
    const valueSummary = {
...
//# sourceMappingURL=calculate_realized_value.js.map