...
export const GENERATE_EXECUTIVE_SUMMARY_TOOL = {
    ...
    description: "Generates a comprehensive one-page executive summary combining AI readiness assessment, bottleneck analysis, pilot plan, and ROI projections into a decision-ready document for leadership.",
    inputSchema: {
        type: "object",
        properties: {
...
function calculateRiskLevel(readinessScore, complexity, redFlagsCount) {
    let riskScore = 0;
    if (readinessScore < 4) {
        riskScore += 2;
    }
...
function generateOpportunities(bottlenecks, industry) {
    const opportunities = [];
    ...
    bottlenecks
        ...
        .slice(0, 2)
        .forEach((b) => {
        opportunities.push(`${b.name}: ${b.recommended_ai_approach} (${Math.round(b.estimated_annual_cost_usd / 1000)}K annual impact)`);
    });
...
function generateTimeline(durationWeeks, complexity) {
...
function generateRiskSummary(readinessAssessment, complexity) {
    const risks = [];
...
function generateNextSteps(hasExecutiveSponsor) {
    const steps = [
...
export function generateExecutiveSummary(input) {
    const { company_name, industry, assessment_date, readiness_assessment, top_bottlenecks, pilot_plan, roi_projection, executive_sponsor, prepared_by, } = input;
    ...
    return {
        ...
        snapshot: {
            ...
            risk_level: calculateRiskLevel(readiness_assessment.overall_score, complexity, readiness_assessment.red_flags.length),
        },
        key_findings: {
        ...
        methodology_note: "This assessment follows Good AI methodology: 'Leverage, not lore' (immediate value), 'Evidence over opinions' (data-driven), 'Augment first' (enhance existing capabilities), and 'Non-invasive by default' (minimal disruption).",
    };
}
//# sourceMappingURL=generate_executive_summary.js.map