...
// Scoring Functions
// ============================================
function calculateDataReadiness(input) {
    ...
    if (input.data_infrastructure.data_quality_score &&
...
function calculateTechnicalCapability(input) {
...
function calculateProcessMaturity(input) {
...
function calculateOrganizationalReadiness(input) {
    let score = 2; // Base score (calibrated for realistic assessment)
    const findings = [];
...
function identifyStrengths(input, dataReadiness, technicalCapability) {
    const strengths = [];
    if (input.data_infrastructure.centralized_data) {
...
function determineRedFlags(overallScore, input) {
    const redFlags = [];
...
export function assessAIReadiness(input) {
    ...
    const overallScore = (dataReadiness.score +
    ...
    // Get recommendation
    const recommendedStartingPoint = getIndustryRecommendation(input.industry, input.data_infrastructure.centralized_data, input.data_infrastructure.manual_data_entry_percent);
    // Calculate time to value
    const estimatedTimeToValueWeeks = calculateTimeToValue(input.industry, input.data_infrastructure.centralized_data, input.current_systems.legacy_systems_count, input.data_infrastructure.manual_data_entry_percent);
    return {
...
export const ASSESS_AI_READINESS_TOOL = {
    ...
    description: "Evaluate an organization's readiness for AI implementation. Provides scores across data readiness, technical capability, process maturity, and organizational readiness dimensions. Returns actionable recommendations based on Good AI's 'Leverage, not lore' methodology.",
    inputSchema: {
        type: "object",
        properties: {
            company_name: {
...
//# sourceMappingURL=assess_ai_readiness.js.map