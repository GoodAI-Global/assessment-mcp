...
export const MEASURE_ADOPTION_TOOL = {
    ...
    description: "Measures and analyzes AI solution adoption rates, user engagement, and behavioral patterns. Provides adoption health scores, risk indicators, and recommendations for improving adoption.",
    inputSchema: {
        type: "object",
        properties: {
...
// ============================================
function calculateDaysSinceGoLive(goLiveDate, endDate) {
...
function identifyAdoptionRisks(input, adoptionRate, engagementScore) {
    ...
    const daysSinceGoLive = calculateDaysSinceGoLive(deployment_info.go_live_date, input.assessment_period.end_date);
    // Low adoption rate
...
function generateSuccessIndicators(adoptionRate, engagementScore, daysSinceGoLive) {
    const achieved = [];
    const upcoming = [];
...
function generateTrendAnalysis(input, adoptionRate, velocity) {
    ...
    if (benchmarks?.previous_period_active_users !== undefined) {
        const previousRate = calculateAdoptionRate(benchmarks.previous_period_active_users, input.deployment_info.total_target_users);
        ...
        projectedRate = Math.min(100, adoptionRate * (velocityMultipliers[velocity] || 1));
        confidence = "low";
    }
    // Inflection points
...
export function measureAdoption(input) {
    const { project_name, client_name, solution_name, deployment_info, usage_metrics, engagement_indicators, benchmarks, assessment_period, } = input;
    ...
    const velocity = determineAdoptionVelocity(adoptionRate, benchmarks?.previous_period_active_users
        ? calculateAdoptionRate(benchmarks.previous_period_active_users, deployment_info.total_target_users)
        : undefined, daysSinceGoLive);
    // Determine adoption stage and health
    const adoptionStage = determineAdoptionStage(adoptionRate, daysSinceGoLive, velocity);
    const adoptionHealth = determineAdoptionHealth(adoptionRate, engagement_indicators?.user_satisfaction_score, engagement_indicators?.support_tickets_per_week, deployment_info.total_target_users);
    ...
    const supportBurden = determineSupportBurden(engagement_indicators?.support_tickets_per_week, usage_metrics.active_users_count);
    // Behavioral insights
    ...
    return {
        ...
        methodology_note: "Adoption measurement follows Good AI Client Success methodology, incorporating user engagement analytics, behavioral patterns, and industry benchmarks. Metrics are calculated based on active usage data and contextual factors. Regular measurement cadence (weekly/monthly) is recommended for trend accuracy.",
    };
}
//# sourceMappingURL=measure_adoption.js.map