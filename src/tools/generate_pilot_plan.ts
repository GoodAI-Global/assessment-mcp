...
function generateSuccessMetrics(bottleneckName: string, approach: string): string[] {
...
function generateRiskMitigation(
  complexity: "low" | "medium" | "high",
  hasStakeholderApproval: boolean
): string[] {
  const risks: string[] = [];
...
export function generatePilotPlan(
  ...
  const riskMitigation = generateRiskMitigation(
    complexity,
    constraints.required_stakeholder_approval ?? false
  );
  ...
  const methodologyNotes =
    `This pilot follows Good AI's core principles:\n` +
...
export const GENERATE_PILOT_PLAN_TOOL = {
  ...
  description:
    "Create a detailed pilot implementation plan following Good AI methodology. Generates milestones, success metrics, risk mitigation strategies, and next steps.",
  inputSchema: {
