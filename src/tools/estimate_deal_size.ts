...
export const ESTIMATE_DEAL_SIZE_TOOL = {
  ...
  description:
    "Estimates the potential deal value for AI consulting engagements based on company profile, engagement scope, complexity factors, and market positioning. Provides detailed pricing breakdown, margin analysis, and negotiation guidance.",
  inputSchema: {
    type: "object",
    properties: {
...
const BASE_PRICING: Record<string, { base: number; perUser: number; perIntegration: number }> = {
...
function getCompanySizeMultiplier(employeeCount: number): number {
...
function calculateComplexityAdjustment(
  baseValue: number,
  factors: EstimateDealSizeInput["complexity_factors"]
): { adjustment: number; factors: DealSizeEstimate["pricing_factors"] } {
  const pricingFactors: DealSizeEstimate["pricing_factors"] = [];
  ...
  const regulatoryMultipliers: Record<string, number> = {
    none: 0,
    standard: 0.1,
    strict: 0.25,
    critical: 0.4,
...
function calculateScopeAdjustment(
  baseValue: number,
  scope: EstimateDealSizeInput["scope_dimensions"],
...
function calculateTimelineAdjustment(
  baseValue: number,
  timeline: string
): { adjustment: number; factor: DealSizeEstimate["pricing_factors"][0] | null } {
...
function generateServicesBreakdown(
  totalValue: number,
  engagementType: string,
  hasCustomDev: boolean
): DealSizeEstimate["deal_structure"]["services_breakdown"] {
  const breakdowns: Record<string, { category: string; percentage: number }[]> = {
    assessment: [
...
function generatePhases(
  ...
  engagementType: string,
  timeline: string
): DealSizeEstimate["deal_structure"]["recommended_phases"] {
  const timelineMultiplier = timeline === "accelerated" ? 0.7 : timeline === "extended" ? 1.3 : 1.0;

  const phaseTemplates: Record<
    string,
    { phase: string; weeksPct: number; valuePct: number; deliverables: string[] }[]
  > = {
    assessment: [
      {
        phase: "Discovery",
...
function calculateMarginAnalysis(
  totalValue: number,
  engagementType: string,
  complexity: EstimateDealSizeInput["complexity_factors"]
): DealSizeEstimate["margin_analysis"] {
  ...
  const marginRisk: "low" | "medium" | "high" =
    marginPercent >= 0.45 ? "low" : marginPercent >= 0.35 ? "medium" : "high";
...
function calculateMarketRates(
...
): DealSizeEstimate["competitive_positioning"] {
  // Market variance by industry
  const industryVariance: Record<string, number> = {
    manufacturing: 0.15,
    insurance: 0.2,
    aquaculture: 0.25,
    healthcare: 0.2,
  ...
  const positioningMap: Record<string, "value" | "market" | "premium"> = {
    sole_source: "premium",
    preferred: "premium",
    competitive: "market",
...
function calculateExpansionPotential(
  ...
  _industry: string
): DealSizeEstimate["expansion_potential"] {
  const expansionMultipliers: Record<string, { y1: number; y2: number; y3: number }> = {
...
/** Generate deal risks */
function generateDealRisks(input: EstimateDealSizeInput): DealSizeEstimate["deal_risks"] {
  const risks: DealSizeEstimate["deal_risks"] = [];
...
function generateNegotiationGuidance(
  totalValue: number,
  _input: EstimateDealSizeInput
): DealSizeEstimate["negotiation_guidance"] {
  ...
  const objections: DealSizeEstimate["negotiation_guidance"]["common_objections"] = [
    {
      objection: "Price is higher than other vendors",
      response:
        "Our pricing reflects proven delivery and reduced risk. Our methodology ensures faster time-to-value.",
    },
    {
      objection: "Budget constraints this quarter",
      response:
        "We can phase the engagement or adjust scope while maintaining core value delivery.",
    },
    {
    ...
    },
    {
      ...
      response:
        "Our accelerators and industry expertise reduce time-to-value by 40% compared to internal builds.",
    },
  ];
...
function generateRecommendations(
  input: EstimateDealSizeInput,
  estimate: DealSizeEstimate["estimate"],
  _positioning: DealSizeEstimate["competitive_positioning"]
): string[] {
  ...
  if (input.engagement_type === "assessment") {
    recommendations.push(
      "Include clear path to implementation to maximize assessment-to-pilot conversion"
    );
...
export function estimateDealSize(input: EstimateDealSizeInput): DealSizeEstimate {
  const {
    company_name,
    industry,
    employee_count,
  ...
  const { adjustment: complexityAdjustment, factors: complexityFactors } =
    calculateComplexityAdjustment(baseValue, complexity_factors);

  const scopeAdjustment = calculateScopeAdjustment(baseValue, scope_dimensions, engagement_type);

  const { adjustment: timelineAdjustment, factor: timelineFactor } = calculateTimelineAdjustment(
    baseValue + complexityAdjustment + scopeAdjustment,
    timeline_preference
  );
  ...
  const confidence: "low" | "medium" | "high" =
    confidenceScore >= 0.9 ? "high" : confidenceScore >= 0.75 ? "medium" : "low";
  ...
  const pricingFactors: DealSizeEstimate["pricing_factors"] = [...complexityFactors];
  if (timelineFactor) {
  ...
  const servicesBreakdown = generateServicesBreakdown(
    totalValue,
    engagement_type,
    scope_dimensions.custom_development_required
  );
  ...
  const expansionPotential = calculateExpansionPotential(
    totalValue,
    engagement_type,
    employee_count,
    industry
  ...
  const estimate: DealSizeEstimate["estimate"] = {
  ...
  return {
    ...
    methodology_note:
      "Deal sizing follows Good AI methodology: evidence-based pricing using industry benchmarks, complexity analysis, and market positioning. Estimates include risk-adjusted ranges to account for scope uncertainty.",
  };
}
