...
export interface TeamCompositionRecommendation {
  ...
  skill_coverage: {
    ...
    skill_overlaps: { skill: string; covered_by: string[]; benefit: string }[];
  };

  /** Team dynamics */
  team_dynamics: {
    leadership_structure: string;
    reporting_hierarchy: { role: string; reports_to: string }[];
    collaboration_model: string;
    communication_cadence: string;
    decision_authority: { area: string; authority: string }[];
  };
...
export const RECOMMEND_TEAM_COMPOSITION_TOOL = {
  ...
  description:
    "Recommends optimal team composition for AI consulting engagements based on project complexity, duration, industry, and client context. Provides detailed staffing plans with roles, skills, and phased allocation.",
  inputSchema: {
    type: "object",
    properties: {
...
interface RoleDefinition {
  role: string;
  seniority: "principal" | "senior" | "mid" | "junior";
  hourlyRate: number;
  skills: string[];
...
function getBaseTeamForEngagement(
  engagementType: string,
  complexity: RecommendTeamCompositionInput["complexity"],
  durationWeeks: number
): { role: string; fteAllocation: number; startWeek: number; endWeek: number }[] {
  const team: { role: string; fteAllocation: number; startWeek: number; endWeek: number }[] = [];
  const isComplex =
    complexity.technical_complexity === "high" ||
    complexity.integration_scope === "enterprise_wide";
  const hasML = complexity.ai_components.some((c) =>
    ["ml_models", "nlp", "computer_vision", "predictive_analytics", "generative_ai"].includes(c)
  );
  const hasData = complexity.ai_components.some((c) =>
    ["data_engineering", "business_intelligence"].includes(c)
  );
...
function addSpecializedRoles(
  team: { role: string; fteAllocation: number; startWeek: number; endWeek: number }[],
  input: RecommendTeamCompositionInput
): void {
  ...
  if (
    client_context.change_management_needs === "significant" &&
  ...
  if (
    client_context.regulatory_requirements === "strict" ||
    industry === "healthcare" ||
    industry === "insurance"
  ) {
    team.push({
  ...
  if (
    complexity.ai_components.includes("nlp") ||
...
function applyConstraints(
  team: { role: string; fteAllocation: number; startWeek: number; endWeek: number }[],
  constraints: RecommendTeamCompositionInput["constraints"]
): void {
  if (!constraints) {
    return;
  }
...
function buildRecommendedTeam(
  baseTeam: { role: string; fteAllocation: number; startWeek: number; endWeek: number }[],
  preferences: RecommendTeamCompositionInput["preferences"],
  durationWeeks: number
): TeamCompositionRecommendation["recommended_team"] {
...
function getSubstitutionOptions(role: string): string[] {
...
function calculatePhasedStaffing(
  team: TeamCompositionRecommendation["recommended_team"],
  durationWeeks: number,
  engagementType: string
): TeamCompositionRecommendation["phased_staffing"] {
  // Define phases based on engagement type
  let phases: { name: string; startPercent: number; endPercent: number; objectives: string[] }[];
...
function analyzeSkillCoverage(
  team: TeamCompositionRecommendation["recommended_team"],
  complexity: RecommendTeamCompositionInput["complexity"]
): TeamCompositionRecommendation["skill_coverage"] {
  ...
  // Check coverage
  const requiredSkills: TeamCompositionRecommendation["skill_coverage"]["required_skills"] = [];
  const skillGaps: TeamCompositionRecommendation["skill_coverage"]["skill_gaps"] = [];
  ...
  neededSkills.forEach((skill) => {
    ...
    if (coveredBy.length === 0) {
      skillGaps.push({
        skill,
        gap_severity: ["Machine learning", "Enterprise architecture", "Data pipelines"].includes(
          skill
        )
          ? "critical"
          : "moderate",
...
function buildTeamDynamics(
  team: TeamCompositionRecommendation["recommended_team"],
  engagementType: string
): TeamCompositionRecommendation["team_dynamics"] {
  ...
  let leadershipStructure: string;
  if (hasDirector) {
    leadershipStructure =
  ...
  const hierarchy: { role: string; reports_to: string }[] = [];
  ...
  const otherRoles = team.filter(
    (t) => t.seniority_level === "mid" || t.seniority_level === "junior"
  );
  ...
  otherRoles.forEach((r) => {
    const reportTo = seniorRoles.find(
      (sr) =>
        sr.role.includes("Manager") || sr.role.includes("Lead") || sr.role.includes("Architect")
    );
    hierarchy.push({
      role: r.role,
  ...
  const decisionAuthority = [
    { area: "Strategic direction", authority: principalRole?.role || "Engagement Partner" },
    {
      ...
      authority:
        team.find((t) => t.role.includes("Architect") || t.role.includes("Lead"))?.role ||
        "Technical Lead",
    },
    {
...
function identifyStaffingRisks(
  team: TeamCompositionRecommendation["recommended_team"],
  skillCoverage: TeamCompositionRecommendation["skill_coverage"],
  input: RecommendTeamCompositionInput
): TeamCompositionRecommendation["staffing_risks"] {
  const risks: TeamCompositionRecommendation["staffing_risks"] = [];
...
function generateAlternatives(
  _team: TeamCompositionRecommendation["recommended_team"],
  _input: RecommendTeamCompositionInput
): TeamCompositionRecommendation["alternative_configurations"] {
  return [
    {
      name: "Lean Team",
    ...
    },
    {
      ...
      recommended_when:
        "Cost-sensitive engagement, strong internal standards, adequate senior oversight",
    },
    {
...
function generateRecommendations(
  skillCoverage: TeamCompositionRecommendation["skill_coverage"],
  input: RecommendTeamCompositionInput
): TeamCompositionRecommendation["recommendations"] {
  const hiring: string[] = [];
...
export function recommendTeamComposition(
  ...
  const avgHourlyRate = Math.round(
    recommendedTeam.reduce((sum, m) => sum + m.billable_rate_usd_per_hour * m.fte_allocation, 0) /
      totalFTE
  );
  ...
  let teamLevel: "senior_heavy" | "balanced" | "junior_heavy";
  ...
  return {
    ...
    summary: {
      ...
      key_staffing_rationale:
        `Team composed for ${engagement_type} engagement with ${complexity.technical_complexity} technical complexity. ` +
    ...
    methodology_note:
      "Team composition follows Good AI staffing methodology based on engagement type, complexity, and client context. Roles and allocations are calibrated against industry benchmarks and delivery best practices. Regular team composition reviews are recommended as project needs evolve.",
  };
}
