...
export const PREDICT_CHURN_RISK_TOOL = {
  ...
  description:
    "Predicts client churn risk based on engagement, satisfaction, value realization, and relationship health metrics. Provides risk scores, warning signals, and retention strategies.",
  inputSchema: {
    type: "object",
    properties: {
...
function calculateDaysUntilContractEnd(endDate: string): number {
...
function calculateEngagementScore(
  metrics: PredictChurnRiskInput["engagement_metrics"],
  accountInfo: PredictChurnRiskInput["account_info"]
): number {
...
function calculateSatisfactionScore(
  indicators: PredictChurnRiskInput["satisfaction_indicators"]
): number {
  let score = 50;
...
function calculateOverallChurnScore(
  engagementScore: number,
  satisfactionScore: number,
  valueScore: number,
  relationshipScore: number,
  externalFactors: PredictChurnRiskInput["external_factors"],
  recentEvents: PredictChurnRiskInput["recent_events"]
): number {
...
function calculateRenewalProbability(churnScore: number): number {
...
function determineUrgency(
  churnScore: number,
  daysUntilEnd: number
): "immediate" | "this_quarter" | "this_half" | "monitor" {
...
function identifyWarningSignals(
  input: PredictChurnRiskInput,
  _churnScore: number
): ChurnRiskPrediction["warning_signals"] {
  const signals: ChurnRiskPrediction["warning_signals"] = [];
...
function generateRetentionStrategy(
  input: PretentionChurnRiskInput,
  churnScore: number,
  daysUntilEnd: number
): ChurnRiskPrediction["retention_strategy"] {
  const priorityActions: ChurnRiskPrediction["retention_strategy"]["priority_actions"] = [];
  const relationshipActions: string[] = [];
  const valueActions: string[] = [];
  const execActions: string[] = [];
  const savePlays: string[] = [];
...
type PretentionChurnRiskInput = PredictChurnRiskInput;

function generateScenarios(
  churnScore: number,
  input: PredictChurnRiskInput
): ChurnRiskPrediction["scenarios"] {
...
function assessCompetitiveThreat(
  input: PredictChurnRiskInput,
  industry: string
): ChurnRiskPrediction["competitive_assessment"] {
...
function identifySuccessIndicators(
  input: PredictChurnRiskInput
): ChurnRiskPrediction["success_indicators"] {
...
export function predictChurnRisk(
  ...
  const churnScore = calculateOverallChurnScore(
    engagementScore,
    satisfactionScore,
    valueScore,
    relationshipScore,
