...
export const PRIORITIZE_USE_CASES_TOOL = {
  ...
  description:
    "Ranks potential AI projects by business value, feasibility, and strategic fit. Creates a phased implementation roadmap with ROI projections and resource requirements.",
  inputSchema: {
    type: "object",
    properties: {
...
const INDUSTRY_VALUE_MULTIPLIERS: Record<Industry, Record<string, number>> = {
  manufacturing: {
    automation: 1.3,
    prediction: 1.2,
    optimization: 1.4,
...
function estimateDefaultValue(category: string, industry: Industry): number {
...
function estimateDefaultCost(category: string, complexity?: string): number {
...
function calculateStrategicFit(useCase: UseCase): number {
  let score = 5;
...
function calculateQuickWinPotential(useCase: UseCase): number {
  let score = 5;
...
function generateRisks(useCase: UseCase): string[] {
  const risks: string[] = [];
...
function generatePrerequisites(useCase: UseCase): string[] {
...
function generateSuccessFactors(useCase: UseCase): string[] {
...
function determineTier(overall: number): ScoredUseCase["priority_tier"] {
  if (overall >= 8) {
    return "critical";
  }
  if (overall >= 6.5) {
...
export function prioritizeUseCases(input: PrioritizeUseCasesInput): UseCasePrioritization {
  const {
    ...
    organizational_readiness_score = 5,
  } = input;
  ...
  const prioritized: ScoredUseCase[] = scored.map((item, index) => {
    const rank = index + 1;
  ...
  const getPhaseMetrics = (cases: ScoredUseCase[]) => {
  ...
  if (!fitsBudget && budget_constraint_usd) {
    adjustments.push(
      `Consider phasing Phase 1 implementation to stay within ${budget_constraint_usd.toLocaleString()} budget`
    );
  }
  ...
  if (phase1Cases.length > 0) {
    recommendations.push(
      `Start with ${phase1Cases[0].name} — highest priority with ${phase1Cases[0].estimated_roi_percent}% expected ROI`
    );
  }
  if (quickWins > 0) {
  ...
  return {
    ...
    methodology_note:
      "Prioritization uses weighted scoring across business value, feasibility, strategic fit, and quick-win potential following Good AI methodology: Leverage, not lore.",
  };
}
