...
export const QUALIFY_LEAD_TOOL = {
  name: "qualify_lead",
  description:
    "Scores prospect fit based on company profile using BANT framework (Budget, Authority, Need, Timeline). Provides qualification tier, deal potential, and recommended next steps for Sales.",
  inputSchema: {
    type: "object",
    properties: {
...
const IDEAL_COMPANY_SIZE: Record<Industry, { min: number; max: number; ideal: number }> = {
...
const CORE_INDUSTRIES: Industry[] = ["manufacturing", "insurance", "aquaculture"];

function calculateFitScore(input: QualifyLeadInput): number {
  let score = 5;
...
function calculateBudgetScore(input: QualifyLeadInput): number {
...
function calculateAuthorityScore(input: QualifyLeadInput): number {
  let score = 4;
...
function calculateNeedScore(input: QualifyLeadInput): number {
  let score = 3;
...
function calculateTimingScore(input: QualifyLeadInput): number {
...
function assessCompanySizeFit(
  employees: number,
  industry: Industry
): LeadQualification["fit_assessment"]["company_size_fit"] {
...
function assessTechnologyReadiness(
  maturity?: string
): LeadQualification["fit_assessment"]["technology_readiness"] {
  if (maturity === "high") {
    return "ready";
  }
  if (maturity === "medium") {
...
function generateStrengths(input: QualifyLeadInput, scores: LeadQualification["scores"]): string[] {
...
function generateConcerns(input: QualifyLeadInput, scores: LeadQualification["scores"]): string[] {
...
function generateRecommendedActions(
  tier: LeadQualification["qualification_tier"],
  input: QualifyLeadInput,
  scores: LeadQualification["scores"]
): string[] {
...
function estimateDealSize(input: QualifyLeadInput): LeadQualification["deal_potential"] {
  ...
  // Determine deal type
  let dealType: LeadQualification["deal_potential"]["deal_type"] = "pilot";
  if (baseSize >= 500000) {
    dealType = "strategic";
...
function generateNextSteps(
  tier: LeadQualification["qualification_tier"],
  input: QualifyLeadInput
): LeadQualification["next_steps"] {
  const steps: LeadQualification["next_steps"] = [];
...
function generateDisqualificationReasons(
  scores: LeadQualification["scores"],
  input: QualifyLeadInput
): string[] {
...
export function qualifyLead(input: QualifyLeadInput): LeadQualification {
  const scores = {
