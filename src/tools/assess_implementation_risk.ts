...
export interface ImplementationRiskAssessment {
  ...
  risk_summary: {
    overall_risk_score: number; // 1-10, higher = more risky
  ...
  /** Mitigation plan */
  mitigation_plan: {
    immediate_actions: { action: string; priority: "high" | "medium"; effort: string }[];
    short_term_actions: { action: string; priority: "high" | "medium" | "low"; timeline: string }[];
    ongoing_monitoring: { item: string; frequency: string; owner: string }[];
  };
...
export const ASSESS_IMPLEMENTATION_RISK_TOOL = {
  ...
  description:
    "Assesses implementation risks for AI projects across technical, organizational, external, and execution dimensions. Provides risk scores, mitigation strategies, and governance recommendations.",
  inputSchema: {
    type: "object",
    properties: {
...
function calculateOrganizationalRisk(
  factors: AssessImplementationRiskInput["organizational_factors"],
  projectType: string
): ImplementationRiskAssessment["risk_categories"]["organizational_risk"] {
  const riskFactors: { factor: string; impact: number; description: string }[] = [];
  let totalScore = 0;
...
function calculateExternalRisk(
  factors: AssessImplementationRiskInput["external_factors"],
  industry: string
): ImplementationRiskAssessment["risk_categories"]["external_risk"] {
  const riskFactors: { factor: string; impact: number; description: string }[] = [];
  let totalScore = 0;

  const externalFactors = factors || {
    regulatory_requirements: "none" as const,
    vendor_dependencies: 0,
    market_pressure: "medium" as const,
    economic_uncertainty: "medium" as const,
  };
...
function calculateExecutionRisk(
  teamFactors: AssessImplementationRiskInput["team_factors"],
  budgetUsd: number,
  durationWeeks: number,
  knownIssues: AssessImplementationRiskInput["known_issues"]
): ImplementationRiskAssessment["risk_categories"]["execution_risk"] {
  const riskFactors: { factor: string; impact: number; description: string }[] = [];
  let totalScore = 0;

  const team = teamFactors || {
    team_experience_level: "experienced" as const,
    team_stability: "stable" as const,
...
function generateIdentifiedRisks(
  categories: ImplementationRiskAssessment["risk_categories"],
  _input: AssessImplementationRiskInput
): ImplementationRiskAssessment["identified_risks"] {
  const risks: ImplementationRiskAssessment["identified_risks"] = [];
  let riskId = 1;

  // Add risks from each category
  const categoryMappings: {
    category: "technical" | "organizational" | "external" | "execution";
    data: typeof categories.technical_risk;
  }[] = [
...
function getOwnerRecommendation(category: string): string {
...
): ImplementationRiskAssessment["risk_heatmap"] {
  return {
...
function generateMitigationPlan(
  risks: ImplementationRiskAssessment["identified_risks"]
): ImplementationRiskAssessment["mitigation_plan"] {
...
function generateRecommendations(
  ...
  let proceedRec: "proceed" | "proceed_with_caution" | "reassess" | "do_not_proceed";
...
function generateGovernance(
  overallLevel: string,
  projectType: string
): ImplementationRiskAssessment["governance_recommendations"] {
...
export function assessImplementationRisk(
  input: AssessImplementationRiskInput
  ...
  return {
    ...
    methodology_note:
      "Risk assessment follows Good AI methodology with structured evaluation across technical, organizational, external, and execution dimensions. Scores are calculated using weighted factors and industry benchmarks. Regular reassessment is recommended as project conditions change.",
  };
}
