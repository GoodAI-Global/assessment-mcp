...
export interface DataSourceAssessment {
  ...
  quality_dimensions: {
    completeness: { score: number; finding: string };
    freshness: { score: number; finding: string };
    accessibility: { score: number; finding: string };
    consistency: { score: number; finding: string };
  };
  risks: string[];
...
export const ASSESS_DATA_QUALITY_TOOL = {
  ...
  description:
    "Deep analysis of data assets for AI readiness. Evaluates data sources across completeness, freshness, accessibility, and consistency dimensions. Identifies gaps and provides remediation recommendations.",
  inputSchema: {
    type: "object",
    properties: {
...
function assessFreshness(
  ...
  const scores: Record<string, { score: number; finding: string }> = {
...
function generateSourceRisks(
  source: z.infer<typeof DataSourceSchema>,
  assessment: DataSourceAssessment["quality_dimensions"]
): string[] {
  const risks: string[] = [];
...
function assessDataSource(
  ...
  const avgScore =
    (completeness.score + freshness.score + accessibility.score + consistency.score) / 4;
...
function identifyDataGaps(
  sources: DataSourceAssessment[],
  input: AssessDataQualityInput
): DataQualityAssessment["data_gaps"] {
  ...
  const hasHistory = input.data_sources.some(
    (s) => s.historical_depth_months && s.historical_depth_months >= 12
  );
  if (!hasHistory) {
    gaps.push({
...
function assessGovernance(
  input: AssessDataQualityInput
): DataQualityAssessment["governance_assessment"] {
  let score = 3;
  const findings: string[] = [];
  const recommendations: string[] = [];
...
function assessIntegration(
  input: AssessDataQualityInput,
  sources: DataSourceAssessment[]
): DataQualityAssessment["integration_assessment"] {
  const complexity = input.integration_complexity || "medium";
  const challenges: string[] = [];

  // Identify integration challenges
  const manualSources = input.data_sources.filter(
    (s) => s.type === "manual_entry" || s.type === "spreadsheet"
  );
...
function identifyQuickWins(
  ...
  if (readySources.length > 0) {
    quickWins.push(
      `Start with ${readySources[0].name} - already AI-ready (score: ${readySources[0].ai_readiness_score})`
    );
  }
...
function identifyCriticalActions(
  sources: DataSourceAssessment[],
  gaps: DataQualityAssessment["data_gaps"]
): string[] {
...
export function assessDataQuality(
  ...
  const sourcesNeedsWork = assessedSources.filter(
    (s) => s.ai_readiness_score >= 4 && s.ai_readiness_score < 7
  ).length;
  const sourcesNotReady = assessedSources.filter((s) => s.ai_readiness_score < 4).length;

  const hasRealTime = input.data_sources.some(
    (s) => s.update_frequency === "real_time" || s.update_frequency === "hourly"
  );
  const hasHistory = input.data_sources.some(
    (s) => s.historical_depth_months && s.historical_depth_months >= 12
  );
  ...
  const dataPrepWeeks = Math.ceil(
    dataGaps.reduce((sum, g) => sum + g.effort_weeks, 0) * 0.6 +
      integrationAssessment.estimated_effort_weeks * 0.4
  );

  // Determine overall rating
  let rating: DataQualityAssessment["ai_readiness_rating"];
  if (avgScore >= 8) {
    rating = "excellent";
  } else if (avgScore >= 6) {
    rating = "ready";
