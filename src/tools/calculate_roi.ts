...
function generateSensitivityAnalysis(
  ...
  const conservativeMonthlySavings = calculateMonthlySavings(
    input.current_metrics.process_cost_per_month_usd,
    conservativeImprovement
  );
  const conservativeROI = calculateROIPercent(
    input.implementation_cost_usd,
    conservativeMonthlySavings * horizonMonths,
    ongoingCost * horizonMonths
  );
  const conservativePayback = calculatePaybackPeriod(
    input.implementation_cost_usd,
    conservativeMonthlySavings,
    ongoingCost
  );

  // Expected: 100% of target improvement
  const expectedROI = calculateROIPercent(
    input.implementation_cost_usd,
    expectedMonthlySavings * horizonMonths,
    ongoingCost * horizonMonths
  );
  const expectedPayback = calculatePaybackPeriod(
    input.implementation_cost_usd,
  ...
  const optimisticMonthlySavings = calculateMonthlySavings(
    input.current_metrics.process_cost_per_month_usd,
    optimisticImprovement
  );
  const optimisticROI = calculateROIPercent(
    input.implementation_cost_usd,
    optimisticMonthlySavings * horizonMonths,
    ongoingCost * horizonMonths
  );
  const optimisticPayback = calculatePaybackPeriod(
    input.implementation_cost_usd,
    optimisticMonthlySavings,
    ongoingCost
  );
...
function generateAssumptions(input: CalculateROIInput): string[] {
...
export function calculateROI(input: CalculateROIInput): CalculateROIOutput {
  ...
  const hasDetailedMetrics = !!(
    input.current_metrics.error_rate_percent ||
...
export const CALCULATE_ROI_TOOL = {
  name: "calculate_roi",
  description:
    "Calculate return on investment for AI implementations with sensitivity analysis. Provides payback period, NPV, and confidence levels based on input quality.",
  inputSchema: {
