...
export const COMPARE_SCENARIOS_TOOL = {
  ...
  description:
    "Side-by-side comparison of different AI investment scenarios. Evaluates ROI, time to value, risk, and strategic alignment to provide clear recommendation with supporting analysis.",
  inputSchema: {
    type: "object",
    properties: {
...
function calculateTimeToValueScore(scenario: Scenario): number {
...
function calculateStrategicScore(scenario: Scenario): number {
...
function calculateMetrics(scenario: Scenario): ScoredScenario["metrics"] {
...
function generatePros(scenario: Scenario, metrics: ScoredScenario["metrics"]): string[] {
  const pros: string[] = [];
...
function generateCons(scenario: Scenario, metrics: ScoredScenario["metrics"]): string[] {
  const cons: string[] = [];
...
function determineFitAssessment(
  overall: number,
  meetsConstraints: boolean
): ScoredScenario["fit_assessment"] {
  if (!meetsConstraints) {
    return "poor";
  }
  if (overall >= 8) {
...
function generateRiskAnalysis(scenarios: Scenario[]): ScenarioComparison["risk_analysis"] {
...
function generateTradeOffs(scored: ScoredScenario[]): string[] {
  ...
  if (sortedByROI[0].name !== sortedByTime[0].name) {
    tradeoffs.push(
      `${sortedByROI[0].name} offers best ROI but ${sortedByTime[0].name} delivers value faster`
    );
  }
...
export function compareScenarios(input: CompareScenariosInput): ScenarioComparison {
  const {
    company_name,
    comparison_purpose,
    scenarios,
  ...
  const scored = scenarios.map((s) =>
    scoreScenario(s, weights, risk_tolerance, budget_limit_usd, timeline_limit_weeks)
  );
  ...
  const confidenceLevel: "low" | "medium" | "high" =
    winner.scores.overall_score - runnerUp.scores.overall_score > 2
      ? "high"
  ...
  return {
    ...
    methodology_note:
      "Comparison uses weighted multi-criteria decision analysis following Good AI methodology: Evidence over opinions, with transparent scoring across ROI, time to value, risk, and strategic alignment.",
  };
}
